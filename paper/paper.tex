...
are tightly coupled, and no loss weight threads the joint bar at this scale. We
report this negative result in full, argue it is not free only \emph{at small
scale}, and lay out the scale study it motivates. All code, traces, and the
...
\item \textbf{A pre-registered negative result} (\S\ref{sec:sticky}--\ref{sec:results}):
training MoE routers for cacheability works but fails a joint miss/quality bar
at 137M scale; we report the full dose-response and argue for a scale study.
\end{enumerate}
...
\section{sticky-moe: Training for Cacheability}
\label{sec:sticky}
...
b-l20       & B & 0.20 & 70.1 & 0.861 & 0.855 & 0.977 \\
...
\paragraph{Why small scale may be the reason.}
A 16-expert, 137M model has little spare capacity: forcing the router to reuse
or confine experts removes representational freedom the model was using for
language modelling, so quality drops. Sparsity scaling suggests the tax should
shrink with $E$ and total parameters---more, finer experts give the router room
to specialise for caching \emph{and} for the task
\citep{dai2024deepseekmoe,deepseekai2024deepseekv3}. The sharp, testable hypothesis this negative
result earns is therefore: \emph{the perplexity cost of a fixed cacheability
gain decreases with model scale}. Confirming or refuting it needs a size ladder
(e.g.\ 137M$\to$1B$\to$7B), which is the natural follow-up and is inexpensive in
the cloud relative to a frontier run.

\section{Limitations}
\label{sec:limits}
(i) \textbf{Scale}: a single 137M configuration; routers at frontier scale may
behave differently, and this is exactly the open question. (ii) \textbf{Proxy}:
...
exploit. \textbf{Edge MoE and expert offloading.} PowerInfer/PowerInfer-2
\citep{song2024powerinfer,xue2024powerinfer2}, EdgeMoE \citep{yi2023edgemoe}, MoE-Infinity
\citep{xue2024moeinfinity}, HOBBIT \citep{tang2024hobbit}, AdapMoE \citep{zhong2024adapmoe}, and related
systems predict, prefetch, and cache experts across the hierarchy---the lineage
of PMS; MoE-Infinity's activation tracing is the direct ancestor of our tracer.
...
and data hashes are released with this paper.
